...
namespace lve
{
	...
	void LveCamera::SetPerspectiveProjection(F32 fovy, F32 aspect, F32 near, F32 far)
	{
		ASSERT(MathOp::Abs(aspect - std::numeric_limits<F32>::epsilon()) > 0.0f);
	...
	void LveCamera::SetViewDirection(Vector3 position, Vector3 direction, Vector3 up)
	{
		// Orthonormal basis
	...
	void LveCamera::SetViewTarget(Vector3 position, Vector3 target, Vector3 up)
	{
		Vector3 direction = target - position;
		ASSERT(MathOp::Abs(direction.x - std::numeric_limits<F32>::epsilon()) > 0.0f);
		ASSERT(MathOp::Abs(direction.y - std::numeric_limits<F32>::epsilon()) > 0.0f);
		ASSERT(MathOp::Abs(direction.z - std::numeric_limits<F32>::epsilon()) > 0.0f);

		SetViewDirection(position, direction, up);
	}

	void LveCamera::SetViewYXZ(Vector3 position, Vector3 rotation)
	{
